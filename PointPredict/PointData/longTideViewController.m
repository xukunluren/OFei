...
- (void)viewDidLoad {
    ...
    _longTideTable = [[UITableView alloc] initWithFrame:CGRectMake(0,66,KWight,KHight-kDockHeight) style:UITableViewStylePlain];
...
-(NSURL *)judgePoint:(NSString *)point
{
    ...
    if ([point isEqualToString:@"A点"]) {
        url= [NSURL URLWithString:@KPointLTideA];
    }
    if ([point isEqualToString:@"B点"]) {
        url= [NSURL URLWithString:@KPointLTideB];
    }
    if ([point isEqualToString:@"C点"]) {
        url= [NSURL URLWithString:@KPointLTideC];
    }
    if ([point isEqualToString:@"D点"]) {
        url= [NSURL URLWithString:@KPointLTideD];
    }
    return url;
}
